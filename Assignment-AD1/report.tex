...
The joint distribution $p(x,z)$ can be evaluated as $p(x|z) \cdot p(z)$ , the ELBO can be written in the folllowing way ( which is preferred when we believe that $q_\phi(z|x) \approx p(z|x)$)
\[
\elbo(\phi) = E_{z \sim q}\left[ \log p(x|z) + \log p(z) -\log q_\phi(z|x) \right]
\]
Using the reparameterization trick we can express a sample z from a parametric distribution $q_\phi(z)$ as a deterministic functio of a random variable $\epsilon$ with some fixed distribution and the parameters $\phi$ of $q_\phi$ 
\[
z = t(\epsilon , \phi)
\]
The idea behind the Sticking The Landing paper is to apply the reparameterization trick to continous latent variables models in order to get a lower variance gradient estimates ( the reparameterization trick provids more informative gradients by exposing the dependence of sampled latent variables z on variational parameters $\phi$)\\
...
\begin{align*}
\widehat{\nabla}_{\text{TD}}(\epsilon, \phi) &= \nabla_\phi \left[\log p(\mathbf{x}|\mathbf{z}) + \log p(\mathbf{z}) - \log q_\phi(\mathbf{z}|\mathbf{x})\right] \\
&= \nabla_\phi \left[\log p(\mathbf{z}|\mathbf{x}) + \log p(\mathbf{x}) - \log q_\phi(\mathbf{z}|\mathbf{x})\right]  \\
&= \underbrace{\nabla_\mathbf{z} \left[\log p(\mathbf{z}|\mathbf{x}) - \log q_\phi(\mathbf{z}|\mathbf{x})\right] \nabla_\phi t(\epsilon, \phi)}_{\color{blue}\text{path derivative}} - \underbrace{\nabla_\phi \log q_\phi(\mathbf{z}|\mathbf{x})}_{\color{red}\text{score function}}
\end{align*}
The reparameterized gradien estimator wrt $\phi$ decomposes into to parts:
\begin{itemize}
    \item Path derivative $\rightarrow$ measures dependence on $\phi$ only through the sample z 
    \item Score function $\rightarrow$ measures the dependence on $\log q_\phi$ directly, without considering how sample z changes as a function of $\phi$
\end{itemize}
Even the reparameterized gradient estimate contains the score function but it can be easily removed and that doing so gives even lower variance gradient estimates in many circumastances ( if $q_\phi(z|x) = p(z|x)$ the path derivative is equal to 0 for all z meanwhile the score function compoenent is not, meaning that the total derivative gradient estimator will have non zero variance even when $q$ matches the exact posterior everywhere )

\subsubsection*{Question 3.1.22}
We show that the expectaction of the score function is zero.
\begin{align*}
    E_{q(z|x)}\left[ \nabla_\phi \log q_\phi(z|x) \right] &= \int \nabla_\phi \log q_\phi(z|x) q_\phi(z|x) dz\\
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.4\linewidth]{imgs/B2-Algorithm2OfStickingTheLandingPaper.jpeg}
\end{figure}
...
To implement Algorithm 2, we remove the unnecessary score-function component of the reparameterized gradient. Practically, this requires stopping the flow of gradients from the reconstruction term back into the encoder parameters $\phi$\\
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\linewidth]{imgs/B2-ELBO-1.png}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\linewidth]{imgs/B2-ELBO-2.png}
\end{figure}
\textbf{Conclusion} We observe a significant decrease in the Evidence Lower Bound (ELBO) for Algorithm 2 compared to Algorithm 1, despite the former demonstrating lower variance. I attempted several different configurations to improve this outcome. These attempts included detaching only the mean—the variational parameter associated with the high est gradient variance—and decreasing the overall learning rate. However, the result consistently remained the same. This outcome was entirely unexpected, as the goal of employing variance reduction techniques is to stabilize training and lower gradient variance without compromising the quality of the final result. The fact that the ELBO quality severely diminished while the variance decreased suggests that the implementation, although successful in reducing noise, introduced a fundamental instability that prevented convergence to the correct objective function
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\linewidth]{imgs/A1-DGM.jpeg}
\end{figure}
\[
p_\theta(x_{0:T}) = p_\theta(x_T) \cdot p_\theta(x_{t-1} | x_t) \cdot ... \cdot p_\theta(x_{1} | x_0) = p_\theta(x_T) \prod_{t=1}^{T-1} p_\theta(x_{t-1} | x_t)
\]
\[
...
\begin{align*}
\mathbb{E}[-\log p_\theta(x_0)] &= -\log \int_{x_{1:T}} p_\theta(x_{0:T}) \, dx_{1:T} \\
&= -\log \int_{x_{1:T}} q(x_{1:T} \mid x_0) \frac{p_\theta(x_{0:T})}{q(x_{1:T} \mid x_0)} \, dx_{1:T} \\
&= -\log \mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ \frac{p_\theta(x_{0:T})}{q(x_{1:T} \mid x_0)} \right] \\
& \leq -\mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ \log \frac{p_\theta(x_{0:T})}{q(x_{1:T} \mid x_0)} \right] \\
&= \mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ - \log p(x_T) - \sum_{t=1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_t \mid x_{t-1})} \right] \\
&= \mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ - \log p(x_T) - \sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_t \mid x_{t-1})} - \log \frac{p_\theta(x_0 \mid x_1)}{q(x_1 \mid x_0)} \right]
\end{align*}
We apply the following transformation
\[
q(x_t \mid x_{t-1}) = \frac{q(x_{t-1} \mid x_{t} , x_0)}{q(x_t, x_0)}
\]
And 
\begin{align*}
q(x_{t-1} \mid x_{t} , x_0) &= \frac{q(x_{t-1} , x_t , x_0 )}{q(x_t , x_0)}\\
&= \frac{q(x_t | x_{t-1} , x_0) q(x_{t-1},x_0)}{q(x_t,x_0)}\\
&= \frac{q(x_t | x_{t-1}) q(x_{t-1} \mid x_0) q(x_0)}{q(x_t | x_0) q(x_0)}\\
&= \frac{q(x_t | x_{t-1}) q(x_{t-1} \mid x_0)}{q(x_t | x_0)} 
\end{align*}
Because we know that \(q(x_t | x_{t-1} , x_0) = q(x_t | x_{t-1}) \)\\
We can finally invert the equation to retrieve the term $q(x_t \mid x_{t-1})$ as
\[
    q(x_t \mid x_{t-1}) = \frac{q(x_{t-1} | x_t, x_0) q(x_t | x_0)}{q(x_{t-1}| x_0)}
\]
Now, we can come back to the proof  
\begin{align*}
\mathbb{E}[-\log p_\theta(x_0)] & \leq \mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ - \log p(x_T) - \sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_t \mid x_{t-1})} - \log \frac{p_\theta(x_0 \mid x_1)}{q(x_1 \mid x_0)} \right]\\
&=   \mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ - \log p(x_T) - \sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{\frac{q(x_{t-1} | x_t, x_0) q(x_t | x_0)}{q(x_{t-1}| x_0)}} - \log \frac{p_\theta(x_0 \mid x_1)}{q(x_1 \mid x_0)} \right]\\
&=  \mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ - \log p(x_T) - \sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_{t-1} | x_t, x_0)} \frac{q(x_{t-1}| x_0)}{q(x_t | x_0)}  - \log \frac{p_\theta(x_0 \mid x_1)}{q(x_1 \mid x_0)} \right]\\
\end{align*}
We can know divide the sum  $\sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_{t-1} | x_t, x_0)} \frac{q(x_{t-1}| x_0)}{q(x_t | x_0)}$
\[
\sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_{t-1} | x_t, x_0)} \frac{q(x_{t-1}| x_0)}{q(x_t | x_0)} = \sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_{t-1} | x_t, x_0)}  + \sum_{t > 1}^T \log \frac{q(x_{t-1}| x_0)}{q(x_t | x_0)}
\]
We can focus on the second term, trying to expand it
\begin{align*}
\sum_{t=2}^T \log \frac{q(x_{t-1} \mid x_0)}{q(x_t \mid x_0)} &= -\log \prod_{t=2}^T \frac{q(x_{t-1} \mid x_0)}{q(x_t \mid x_0)} \\
&= \log \left( \frac{q(x_1 \mid x_0)}{q(x_2 \mid x_0)} \cdot \frac{q(x_2 \mid x_0)}{q(x_3 \mid x_0)} \cdots \frac{q(x_{T-1} \mid x_0)}{q(x_T \mid x_0)} \right) \\
&= \log \frac{q(x_1 \mid x_0)}{q(x_T \mid x_0)} = -\log q(x_T \mid x_0) +\log q(x_1 \mid x_0)
\end{align*}
We can replace it in the main formula for the proof
\begin{align*}
\mathbb{E}[-\log p_\theta(x_0)] & \leq \mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ - \log p(x_T) - \sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_{t-1} | x_t, x_0)} \frac{q(x_{t-1}| x_0)}{q(x_t | x_0)}  - \log \frac{p_\theta(x_0 \mid x_1)}{q(x_1 \mid x_0)} \right]\\
&=  \mathbb{E}_{q(x_{1:T} \mid x_0)} \left[ - \log \frac{p(x_T)}{q(x_T | x_0)} - \sum_{t > 1}^T \log \frac{p_\theta(x_{t-1} \mid x_t)}{q(x_{t-1} | x_t, x_0)}  - \log p_\theta(x_0 \mid x_1) \right]
\end{align*}
...
\[
= \mathbb{E}_{q_1} [D_{\text{KL}}(q(x_T \mid x_0) \parallel p(x_T))] + \sum_{t>2}^T \mathbb{E}_{q_2} [D_{\text{KL}}(q(x_{t-1} \mid x_t, x_0) \parallel p_\theta(x_{t-1} \mid x_t))] - \mathbb{E}_{q_3} [\log p_\theta(x_0 \mid x_1)]
\]
...
&= \mathbb{E}_{q(x_t|x_0)}\left[D_{\text{KL}}(q(x_{t-1}|x_t, x_0) \| p_{\theta}(x_{t-1}|x_t))\right].
\end{align*}
