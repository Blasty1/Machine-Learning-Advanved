...
\\
&= \sum_{x_2} 
   \mathbb{P}(X_4 = \text{t} \mid X_2 = x_2,\, \textcolor{gray}{X_3 = \text{m},\, X_1 = x_1})\,
...
\subsection*{1.3.9 \quad Datasets}
The function \texttt{generate\_data} is used to produce synthetic observations drawn from a univariate Normal distribution with mean $\mu$ and precision $\tau$. Since the precision is defined as $\tau = 1/\sigma^{2}$, the corresponding standard deviation is computed as $\sigma = 1/\sqrt{\tau}$. A fixed random seed ensures reproducibility of the generated datasets. 
...
\begin{figure}[h!]
    ...
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imgs/data10.png}
        \caption{$N=10$}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imgs/data100.png}
        \caption{$N=100$}
    \end{minipage}
    \hfill
    \begin{minipage}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imgs/data1000.png}
        \caption{$N=1000$}
    \end{minipage}

    \caption{Histograms of the three generated datasets displayed side by side.}
    \label{fig:datasets_hist}
\end{figure}

\subsection*{1.3.10 \quad ML estimates}
The function \texttt{ML\_est} computes the Maximum Likelihood (ML) estimates of the parameters of a univariate Normal distribution given an observed dataset $D$. 
The ML estimate of the mean is obtained as the sample average,
\[
\mu_{\text{ML}} = \frac{1}{N} \sum_{i=1}^{N} x_i,
\]
while the ML estimate of the variance corresponds to the empirical variance computed with \texttt{ddof = 0}, i.e.,
\[
...
\[
\tau_{\text{ML}} = \frac{1}{\sigma^2_{\text{ML}}}.
\]
The function returns the pair $(\mu_{\text{ML}}, \tau_{\text{ML}})$, which serve as point estimates of the true mean and precision underlying the data-generating process.

We got the following results:
\begin{itemize}
  \item $N = 10$: (1.0845817205152941, 0.884224414821371)
  \item $N = 100$: (1.1123121218037397, 0.5346648053646902)
  \item $N = 1000$: (0.9794138084900784, 0.5683337478172712)
\end{itemize}
...
Below we recall the standard CAVI procedure: for each latent variable, we isolate all
terms in the joint density that depend on it, take the expectation with respect to the
other variational factor, and recognise the resulting expression as a known member of
the exponential family. This yields closed-form updates for both $q(\tau)$ and $q(\mu)$ in
the Normal–Gamma model.

\paragraph{Update for $q^*(\tau)$.}
To compute $q^*(\tau)$, we collect all terms in $\log \p (\tau, D)$ that depend on $\tau$ and
take the expectation with respect to $q(\mu)$. The resulting expression contains only a
log-term in $\tau$ and a linear term in $\tau$, which matches the canonical form of a
Gamma distribution. This directly identifies the updated shape and rate parameters,
leading to the expression below.

\begin{align*}
\log q^*(\tau) 
&= \E_{q(\mu)} \left[ \log \p(\tau, D) \right] \\[6pt]
&= \E_{q(\mu)} \left[
\left(a_0 - \dfrac{1}{2}\right) \log \tau 
- \beta_0 \tau 
...
+ \frac{1}{2} \E_{q(\mu)} \left[ \sum_{i=1}^N (x_i - \mu)^2 \right]
\right) \\
...
-\tau \left[
\beta_0
+ \frac{\lambda_0}{2}\E_{q(\mu)}\left[(\mu-\mu_0)^2\right]
+ \frac{1}{2}\E_{q(\mu)} \left[ \sum_{i=1}^N (x_i - \mu)^2\right]
\right]
\right)
\]
...
\paragraph{Update for $q^*(\mu)$.}
For the update of $q(\mu)$, we extract all terms in $\log \p(\mu, D)$ that are
functions of $\mu$ and take the expectation with respect to $q(\tau)$. The dependence on
$\mu$ is entirely quadratic, so the optimal factor must be Gaussian. Identifying the
quadratic coefficient and the linear term immediately yields the updated posterior mean
and precision.

\begin{align*}
\log q^*(\mu)
&= \E_{q(\tau)} \big[ \log \p(\mu, D) \big] \\
&= \E_{q(\tau)} \Big[
-\frac{\lambda_0\tau}{2} (\mu - \mu_0)^2
- \frac{\tau}{2} \sum_{i=1}^N (x_i - \mu)^2
\Big] \\
...
- \frac{\lambda_N}{2} \underbrace{\E_q \big[(\mu - \mu_N)^2\big]}_{\text{Var}_q[\mu]= \frac{1}{\lambda_N}}
\\
&= \frac{1}{2}\log \frac{\lambda_N}{2 \pi} 
...
\item \textbf{Convergence behavior}: The ELBO plot (Figure ~\ref{fig:ELBO}) shows rapid convergence within approximately 5-6 iterations, starting from an initial value of around -0.070 and stabilizing at approximately -0.035. The steep increase in the first iteration followed by marginal improvements indicates that the algorithm efficiently identifies the optimal variational parameters. This fast convergence is characteristic of conjugate variational inference, where coordinate updates have closed-form solutions.
...
\item \textbf{Accuracy of approximation}: The contour plots reveal that the CAVI approximate posterior (green dashed contours) closely matches the exact posterior distribution (blue solid contours) (Figure ~\ref{fig:contour} - ~\ref{fig:ML}) for both the mean parameter \(\mu\) and precision parameter \(\tau\). The numerical comparison confirms this:
