...
\\
&= \dfrac{N}{2} \log \E_q [\tau] - \dfrac{N}{2} \log 2 \pi - \frac{1}{2} \E_{q(\tau)}[\tau]\bigg(\sum^N_{n = 1} x_n^2-2\E_{q(\mu)}[\mu]\sum^N_{n = 1} +\E_{q(\mu)}[\mu^2]\bigg)
\end{align*}

\begin{align*}
\textcolor{orange}{\E_q [\log q (\mu)]} &= \E_q \bigg[\log \bigg(\sqrt{\dfrac{\lambda_N}{2 \pi}} e^{-\frac{\lambda_N}{2}(\mu - \mu_N)^2}\bigg)\bigg] 
\\
&= \E_q \bigg[ \dfrac{1}{2}\log \dfrac{\lambda_N}{2 \pi} - \dfrac{\lambda_N}{2}(\mu - \mu_N)^2 \bigg] = \dfrac{1}{2}\log \dfrac{\lambda_N}{2 \pi} - \dfrac{\lambda_N}{2} \E_q \bigg[(\mu - \mu_N)^2
\\
