...
To compute $q^*(\tau)$, we collect all terms in $\log \p(\tau,X\mid\mu)$ that depend on $\tau$ and
...
\begin{align*}
\log q^*(\tau) 
&= \E_{q(\mu)} \left[ \log p(\tau, X \mid \mu) \right] \\[6pt]
&= \E_{q(\mu)} \left[
...
For the update of $q(\mu)$, we extract all terms in $\log p(\mu,X\mid\tau)$ that are
...
\begin{align*}
\log q^*(\mu)
&= \E_{q(\tau)} \big[ \log p(\mu, X \mid \tau) \big] \\
&= \E_{q(\tau)} \Big[
-\frac{\lambda_0\tau}{2} (\mu - \mu_0)^2
- \frac{\tau}{2} \sum_{i=1}^N (x_i - \mu)^2
\Big] \\
